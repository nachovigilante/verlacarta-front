...
export class OrdersByRestaurantComponent {
  orders: Order[] = [];
  id: string = '';
  orderStates = ['Aceptado', 'En preparación', 'Listo para ser retirado', 'Próximo a ser llevado'];
  ...
  fetchOrders() {
      this.orderService.getOrdersByRestaurantId(this.id).then((data) => (this.orders = data));
  }
